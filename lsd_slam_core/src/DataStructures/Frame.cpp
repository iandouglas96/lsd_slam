...
Frame::Frame(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image, const float* seg)
{
	initialize(id, width, height, K, timestamp);
...
void Frame::buildImage(int level)
{
	...
	for(int y=0;y<wh;y+=width*2)
	{
		...
		{
			s = source + x + y;
			if (s[0] > 0 && s[1] > 0 && s[width] > 0 && s[1+width] > 0) { //Ignore black spaces
				*dest = (s[0] +
						s[1] +
						s[width] +
						s[1+width]) * 0.25f;
